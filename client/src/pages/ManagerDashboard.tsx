...
export default function ManagerDashboard() {
  ...
  const [viewMode, setViewMode] = useState<"daily" | "session">("daily");
  const [selectedSessionForStats, setSelectedSessionForStats] = useState<number | null>(null);
  ...
  // NEW: Payment breakdown by session for selected session (details modal)
  const { data: sessionBreakdownData } = useQuery({
    queryKey: ["/api/manager/payment-breakdown/session", selectedSessionId],
    enabled: !!selectedSessionId,
  });

  // NEW: Payment breakdown by session for statistics view
  const { data: statsSessionBreakdownData } = useQuery({
    queryKey: ["/api/manager/payment-breakdown/session", selectedSessionForStats],
    enabled: !!selectedSessionForStats && viewMode === "session",
  });
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          {viewMode === "session" && (
            <Select value={selectedSessionForStats?.toString() || ""} onValueChange={(value) => setSelectedSessionForStats(parseInt(value))}>
              <SelectTrigger className="w-48 bg-gray-800 border-gray-700 text-white">
                <SelectValue placeholder="Selecionar sessão..." />
              </SelectTrigger>
              <SelectContent className="bg-gray-800 border-gray-700">
                {managerStats?.sessionHistory?.slice(0, 20).map((session: any) => (
                  <SelectItem key={session.id} value={session.id.toString()}>
                    {session.date} - {session.shift} - {session.user} (#{session.id})
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="sales" className="space-y-6">
          ...
          <Card className="bg-gray-800 border-gray-700">
            ...
            <CardContent>
              {/* Payment Breakdown - Dynamic based on view mode */}
              <div className="mb-6">
                {viewMode === "daily" && (
                  <>
                    <h4 className="text-lg font-semibold text-white mb-3">🗓️ Breakdown por Dia: {selectedDate}</h4>
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                      ...
                      <div className="space-y-2">
                        <p className="text-sm font-medium text-cyan-400">📱 Mobile Money</p>
                ...
                {viewMode === "session" && selectedSessionForStats && (
                  <>
                    <h4 className="text-lg font-semibold text-white mb-3">
                      📊 Breakdown por Sessão #{selectedSessionForStats}
                      {statsSessionBreakdownData?.sessionInfo && (
                        <span className="text-sm text-gray-400 ml-2">
                          ({new Date(statsSessionBreakdownData.sessionInfo.startTime).toLocaleDateString('pt-PT')} - {
                            statsSessionBreakdownData.sessionInfo.endTime 
                              ? new Date(statsSessionBreakdownData.sessionInfo.endTime).toLocaleDateString('pt-PT')
                              : 'Em andamento'
                          })
                        </span>
                      )}
                    </h4>
                    {statsSessionBreakdownData ? (
                      <>
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                          <div className="space-y-2">
                            <p className="text-sm font-medium text-green-400">💵 Dinheiro</p>
                            <p className="text-2xl font-bold text-white">{statsSessionBreakdownData.cash.total} F CFA</p>
                            <p className="text-xs text-gray-400">{statsSessionBreakdownData.cash.count} transações</p>
                          </div>
                          <div className="space-y-2">
                            <p className="text-sm font-medium text-cyan-400">📱 Mobile Money</p>
                            <p className="text-2xl font-bold text-white">{statsSessionBreakdownData.mobile.total} F CFA</p>
                            <p className="text-xs text-gray-400">{statsSessionBreakdownData.mobile.count} transações</p>
                          </div>
                          <div className="space-y-2">
                            <p className="text-sm font-medium text-purple-400">💳 Cartão</p>
                            <p className="text-2xl font-bold text-white">{statsSessionBreakdownData.card.total} F CFA</p>
                            <p className="text-xs text-gray-400">{statsSessionBreakdownData.card.count} transações</p>
                          </div>
                          <div className="space-y-2">
                            <p className="text-sm font-medium text-orange-400">📝 Crédito</p>
                            <p className="text-2xl font-bold text-white">{statsSessionBreakdownData.credit.total} F CFA</p>
                            <p className="text-xs text-gray-400">{statsSessionBreakdownData.credit.count} transações</p>
                          </div>
                        </div>
                        <div className="mt-4 pt-4 border-t border-gray-600">
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-lg font-semibold text-white">Total da Sessão:</span>
                            <span className="text-2xl font-bold text-green-400">{statsSessionBreakdownData.total} F CFA</span>
                          </div>
                          <div className="mt-3 p-3 bg-blue-900/20 border border-blue-700 rounded-lg">
                            <p className="text-sm text-blue-300">
                              <strong>💡 Sessão Completa:</strong> Este breakdown inclui TODAS as transações da sessão,
                              incluindo pagamentos de dívidas de crédito e transações que podem ter ocorrido em vários dias.
                            </p>
                          </div>
                        </div>
                      </>
                ...
                {viewMode === "session" && !selectedSessionForStats && (
                  <div className="text-center py-8">
                    <p className="text-gray-400">↑ Selecione uma sessão para ver o breakdown completo</p>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          {/* Enhanced Statistics - Weekly and Credit Reimbursements */}
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              <CardTitle className="text-white flex items-center">
            ...
            <CardContent>
              ...
              {creditPaymentsData && creditPaymentsData.length > 0 && (
                <div className="border-t border-gray-600 pt-4 mt-4">
                  ...
                  <p className="text-xs text-gray-400">{creditPaymentsData.length} pagamentos processados</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          {sessionDetails && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-2">Breakdown de Pagamentos (Sessão Completa)</h3>
                  {sessionBreakdownData ? (
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span className="text-gray-400">💵 Dinheiro ({sessionBreakdownData.cash.count}x):</span>
                        <span className="text-green-400">{formatCurrency(sessionBreakdownData.cash.total)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">📱 Mobile Money ({sessionBreakdownData.mobile.count}x):</span>
                        <span className="text-cyan-400">{formatCurrency(sessionBreakdownData.mobile.total)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">💳 Crédito ({sessionBreakdownData.credit.count}x):</span>
                        <span className="text-blue-400">{formatCurrency(sessionBreakdownData.credit.total)}</span>
                      </div>
                      <div className="border-t border-gray-600 pt-2">
                        <div className="flex justify-between font-semibold">
                          <span className="text-white">Total da Sessão:</span>
                          <span className="text-green-400">{formatCurrency(sessionBreakdownData.total)}</span>
                        </div>
                      </div>
                      <div className="mt-4 p-3 bg-gray-700 rounded-lg">
                        <p className="text-sm text-gray-300 mb-1">
                          <strong>Atenção:</strong> Este breakdown inclui TODAS as transações da sessão completa,
                        </p>
                        <p className="text-xs text-gray-400">
                          mesmo quando a sessão se estende por vários dias.
                          {sessionBreakdownData.sessionInfo && (
                            <span> Sessão {sessionBreakdownData.sessionInfo.id}: {
                              new Date(sessionBreakdownData.sessionInfo.startTime).toLocaleDateString('pt-PT')
                            } - {
                              sessionBreakdownData.sessionInfo.endTime 
                                ? new Date(sessionBreakdownData.sessionInfo.endTime).toLocaleDateString('pt-PT')
                                : 'Em andamento'
                            }</span>
                          )}
                        </p>
