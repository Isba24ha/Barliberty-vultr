...
import { Plus, Search, DollarSign, Calendar, User, CreditCard, Banknote, Smartphone, Minus, Eye, Package } from "lucide-react";
...
export default function Credits() {
  ...
  const handleViewHistory = async (client: CreditClient) => {
    try {
      setSelectedClient(client);
      const response = await apiRequest("GET", `/api/credit-clients/${client.id}/orders`);
      const orders = await response.json();
      setSelectedClientHistory(orders);
  ...
  return (
    <div className="p-6">
      ...
      {/* Order History Modal */}
      <Dialog open={showHistoryModal} onOpenChange={handleCloseHistoryModal}>
          ...
          <div className="space-y-4">
            {selectedClientHistory.length === 0 ? (
              ...
              <div className="space-y-3">
                {selectedClientHistory.map((order: any) => (
                  <Card key={order.id} className="bg-gray-700 border-gray-600">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="text-right">
                          <p className="text-lg font-bold text-orange-400">{order.formattedAmount}</p>
                          {order.tableNumber && (
                            <p className="text-sm text-gray-400">Mesa {order.tableNumber}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="border-t border-gray-600 pt-3 mt-3">
                        <p className="text-sm font-medium text-gray-300 mb-2">Itens do Pedido:</p>
                          ...
                          {order.items.map((item: any, index: number) => (
                            <div key={index} className="flex justify-between text-sm">
                              ...
                              <span className="text-white">
                                {parseFloat(item.total_price || "0").toFixed(2)} F CFA
                              </span>
                            </div>
                          ))}
                        </div>
                      ...
                      {order.payments && order.payments.length > 0 && (
                        <div className="border-t border-gray-600 pt-3 mt-3">
                          <p className="text-sm font-medium text-gray-300 mb-2">Pagamentos:</p>
                          <div className="space-y-1">
                            {order.payments.map((payment: any, index: number) => (
                              <div key={index} className="flex justify-between text-sm">
                                <span className="text-gray-300">
                                  {payment.method === "cash" ? "Dinheiro" : payment.method === "mobile_money" ? "Mobile Money" : "Crédito"}
                                </span>
                                <span className="text-white">
                                  {parseFloat(payment.amount || "0").toFixed(2)} F CFA
                                </span>
                              </div>
                            ))}
                          </div>
                      ...
                      {/* Additional Info */}
                      <div className="border-t border-gray-600 pt-3 mt-3 flex justify-between text-sm text-gray-400">
                        <span>Servidor: {order.serverName}</span>
                        <span>Turno: {order.shiftType}</span>
                      </div>
                    </CardContent>
                  </Card>
                ))}
