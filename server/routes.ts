...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get credit client order history
  app.get("/api/credit-clients/:id/orders", requireAuth, async (req, res) => {
    try {
      ...
      // Get all orders for this credit client with details
      const clientOrders = await db.execute(sql`
        SELECT 
          o.id,
          o.table_id,
          o.total_amount,
          ...
          bs.shift_type,
          bs.start_time as session_start,
          ARRAY_AGG(
            JSON_BUILD_OBJECT(
              'product_name', p.name,
          ...
          ) FILTER (WHERE pm.id IS NOT NULL) as payments
        FROM orders o
        LEFT JOIN tables t ON o.table_id = t.id
        LEFT JOIN users u ON o.server_id = u.id
        LEFT JOIN bar_sessions bs ON o.session_id = bs.id
        LEFT JOIN order_items oi ON o.id = oi.order_id
        LEFT JOIN products p ON oi.product_id = p.id
        LEFT JOIN payments pm ON o.id = pm.order_id
        WHERE o.credit_client_id = ${clientId}
        GROUP BY o.id, t.number, t.location, u.first_name, u.last_name, o.server_id, bs.shift_type, bs.start_time
        ORDER BY o.created_at DESC
      `);

      const formattedOrders = clientOrders.rows.map((row: any) => ({
        id: row.id,
        tableId: row.table_id,
        tableNumber: row.table_number,
        tableLocation: row.table_location,
        totalAmount: parseFloat(row.total_amount || "0").toFixed(2),
        status: row.status,
        createdAt: row.created_at,
        clientName: row.client_name,
        serverName: row.server_name,
        shiftType: row.shift_type === "morning" ? "Manhã" : "Tarde",
        sessionStart: row.session_start,
        items: row.order_items || [],
        payments: row.payments || [],
        formattedDate: new Date(row.created_at).toLocaleDateString('pt-PT'),
        formattedTime: new Date(row.created_at).toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' }),
        formattedAmount: `${parseFloat(row.total_amount || "0").toFixed(2)} F CFA`
      }));

      res.json(formattedOrders);
    } catch (error: any) {
      console.error("Error fetching credit client orders:", error);
      res.status(500).json({ 
  ...
  app.get("/api/manager/credit-client/:id/details", requireAuth, requireRole(["manager"]), async (req, res) => {
    try {
      ...
      // Get payment history (when client paid back debt)
      const paymentHistory = await db
        ...
        .where(
          and(
            eq(payments.creditClientId, clientId),
            eq(payments.isDirectCreditPayment, true) // All direct debt payments
          )
        );
      ...
      res.json({
        ...
        summary: {
          ...
          outstandingBalance: (parseFloat(client.totalCredit) - paymentHistory.reduce((sum, p) => sum + parseFloat(p.amount), 0)).toFixed(2),
        },
      });
    } catch (error) {
  ...
  // Enhanced payment breakdown - BY DATE (original)
    ...
    try {
      ...
      const salesPayments = await db
        ...
        .where(and(
          ...
          eq(payments.isDirectCreditPayment, false) // Only sales payments
        ))
        .groupBy(payments.method);

      // Get debt payments separately
      const debtPayments = await db
        ...
        .where(and(
          gte(payments.createdAt, startDate),
          lt(payments.createdAt, endDate),
          eq(payments.isDirectCreditPayment, true) // Only debt payments
        ))
        .groupBy(payments.method);

      // Combine both types of payments
      const paymentBreakdown = [...salesPayments, ...debtPayments];
  ...
  // NEW: Payment breakdown BY SESSION (correct approach for multi-day sessions)
  app.get("/api/manager/payment-breakdown/session/:sessionId", requireAuth, requireRole(["manager"]), async (req, res) => {
    try {
      ...
      const session = await db
        .select()
        .from(barSessions)
        .where(eq(barSessions.id, parseInt(sessionId)))
        .limit(1);
      ...
      // Get regular sales payments for session
      const salesPayments = await db
        .select({
          method: payments.method,
          amount: sum(payments.amount).as('total'),
        ...
        .from(payments)
        .where(and(
          eq(payments.sessionId, parseInt(sessionId)),
          eq(payments.isDirectCreditPayment, false) // Only sales payments
        ))
        .groupBy(payments.method);

      // Get debt payments for session
      const debtPayments = await db
        .select({
          method: payments.method,
          amount: sum(payments.amount).as('total'),
      ...
      console.error("Error fetching session payment breakdown:", error);
      res.status(500).json({ message: "Erro ao buscar breakdown de pagamentos da sessão" });
    }
  });
