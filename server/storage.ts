  ...
  async getSessionStats(sessionId: number): Promise<SessionStats> {
    ...
    if (!session) {
      return {
        ...
        cashInRegister: "0.00",
        creditPayments: "0.00",
        mobileMoneyPayments: "0.00",
        managerConsumption: "0.00",
        transactionCount: 0,
        activeCredits: "0.00",
        occupiedTables: 0,
        totalTables: 0,
    ...
    // Calculate total sales from all payments during this session
    const totalSalesResult = await db
      .select({ total: sum(sql<number>`CAST(${payments.amount} AS NUMERIC)`) })
      .from(payments)
      .where(eq(payments.sessionId, sessionId));

    // Calculate cash payments (argent liquide en caisse)
    ...
    // Calculate credit payments (paiements de factures clients)
    const creditPaymentsResult = await db
      .select({ total: sum(sql<number>`CAST(${payments.amount} AS NUMERIC)`) })
      .from(payments)
      .where(and(
        eq(payments.sessionId, sessionId),
    ...
    // Calculate mobile money payments
    const mobileMoneyResult = await db
      .select({ total: sum(sql<number>`CAST(${payments.amount} AS NUMERIC)`) })
      .from(payments)
      .where(and(
        eq(payments.sessionId, sessionId),
    ...
    // Calculate manager consumption
    const managerConsumptionResult = await db
      .select({ total: sum(sql<number>`CAST(${payments.amount} AS NUMERIC)`) })
      .from(payments)
      .where(and(
        eq(payments.sessionId, sessionId),
        eq(payments.method, "manager_consumption")
      ));
    ...
    const creditClientsResult = await db
      .select({ total: sum(sql<number>`CAST(${creditClients.totalCredit} AS NUMERIC)`) })
      .from(creditClients)
      .where(eq(creditClients.isActive, true));

    // Convert amounts to proper string format with 2 decimal places
    const totalSales = Number(totalSalesResult[0]?.total || 0).toFixed(2);
    const cashInRegister = Number(cashPaymentsResult[0]?.total || 0).toFixed(2);
    const creditPayments = Number(creditPaymentsResult[0]?.total || 0).toFixed(2);
    const mobileMoneyPayments = Number(mobileMoneyResult[0]?.total || 0).toFixed(2);
    const managerConsumption = Number(managerConsumptionResult[0]?.total || 0).toFixed(2);
    const activeCredits = Number(creditClientsResult[0]?.total || 0).toFixed(2);
    ...
    return {
      ...
      managerConsumption: managerConsumption,
